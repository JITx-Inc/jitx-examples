...
Every number in this file is transcribed from one CSV row of
    ...
    js1-stackup-components/part1-stackup/JS1_Part1_Fab-Stackup.csv
...
* **Reference-plane widths.** ``RoutingStructure.Layer.reference`` requires a
  ...
  the widths below are the substrate-modeler skill's labeled engineering
  default — 3x the dielectric height between line and plane — the one class of
  value in this file that no CSV row backs, labeled as such at every use. See
  ``REF_PLANE_WIDTH``.
...
in ``internal/kits/js1-stackup-components/TODO.md`` along with further findings
...
3. **A reference plane of unstated width still needs a width.**
   ``RoutingStructure.Layer.reference`` takes either a scalar layer plus a
   ``desired_width``, or a mapping of layers to widths. The scalar form without
   a width raises ``TypeError: Must specify desired_width if layer is not a
   mapping``. The mapping form accepts ``None`` values at construction time,
   but they are **translation-fatal**: ``jitx._translate.routing`` assigns
   ``desired_width`` into a protobuf float with no ``None`` guard (verified on
   4.2.2 and 4.4.0rc2). The failure is latent in this kit because routing
   structures only translate when a net, topology or rule applies them, and
   ``main.py``'s smoke design has an empty circuit — it would surface in the
   first consumer design that uses one. So this file carries the planes with
   the substrate-modeler skill's labeled engineering default instead
   (``REF_PLANE_WIDTH``, 3x the dielectric height between line and plane),
   which is rule-backed rather than invented, and satisfies the
   ``Mapping[int, float]`` annotation — no ``pyright`` suppression remains.
"""
...
Passed to ``reference()`` with ``REF_PLANE_WIDTH`` as every desired width. The
report states which planes reference each line and never how wide they are,
and a ``None`` width is translation-fatal (module docstring, ambiguity 3), so
the width is the substrate-modeler skill's labeled default rather than a CSV
value."""

REF_PLANE_WIDTH = 3 * 0.1000  # skill default: 3x dielectric height, not a CSV value
"""Desired reference-plane width: the substrate-modeler skill's engineering
default of 3x the dielectric height between line and plane — a rule-backed
default, not a CSV value, and labeled as such wherever it is used. Every plane
in ``REFERENCE_PLANES`` sits exactly one build-up lamination from its line
(``BuildUpPrepreg.thickness``, the D-BU row's 0.1000 mm), so one constant
covers all of them; a plane further away would sum the dielectric thicknesses
between. ``test_reference_plane_widths_are_the_skill_default`` pins the
arithmetic to the CSV row."""
    ...
    *,
    ...
    return {
        layer: RoutingStructure.Layer(
            ...
            neck_down=neck,
            # The report names the planes, never their width — the width is the
            # skill's 3x-dielectric-height default, not a CSV value.
        ).reference(dict.fromkeys(REFERENCE_PLANES[layer], REF_PLANE_WIDTH))
        for layer in layers
    }
    ...
    *,
    ...
    return {
        layer: DifferentialRoutingStructure.Layer(
            ...
            # Same skill-default width as `_se_layers` — see REF_PLANE_WIDTH.
        ).reference(dict.fromkeys(REFERENCE_PLANES[layer], REF_PLANE_WIDTH))
        for layer in layers
    }
